...
Before running subs2cia, check for subtitles in two places:

**1. External SRT/ASS files** — Check if there are subtitle files alongside the videos in the source directory:
```bash
...
**2. Embedded tracks** — Inspect the video files for embedded audio and subtitle tracks:
```bash
...
If both external and embedded subtitles exist, prefer the external SRT files (they're likely more accurate, especially if generated by the create-srt skill).
